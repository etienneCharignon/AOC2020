...
RSpec.describe RainRisk do
  ...
  describe 'process instructions part 2' do
    it { expect(RainRisk.process("F10", true)).to eql({'N' => 10, 'S' => 0, 'E' => 100, 'W' => 0}) }
    it { expect(RainRisk.process("N1\nF1", true)).to eql({'N' => 2, 'S' => 0, 'E' => 10, 'W' => 0}) }
    it { expect(RainRisk.new.process2_from("R90").waypoint).to eql({'N' => 0, 'S' => 10, 'E' => 1, 'W' => 0}) }
    it { expect(RainRisk.new.process2_from("L90").waypoint).to eql({'N' => 10, 'S' => 0, 'E' => 0, 'W' => 1}) }
    it { expect(RainRisk.process(INPUT12, true)).to eql({'N' => 1649161, 'S' => 1666334, 'E' => 1651476, 'W' => 1665064}) }
  end
end
